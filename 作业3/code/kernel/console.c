...
PUBLIC void out_char(CONSOLE *p_con, char ch)
{
    ...
    case '\b':
        if (!is_esc_and_enter)
        {
            if (p_con->cursor > p_con->original_addr)
            {
                if (*(p_vmem - 1) == DEFAULT_CHAR_COLOR && *(p_vmem - 2) == '\0')
                {
                    while (*(p_vmem - 1) == DEFAULT_CHAR_COLOR && *(p_vmem - 2) == '\0')
                    { //删除/n的\0 即对齐
                        p_con->cursor--;
                ...
                else if (*(p_vmem - 1) == ESC_CHAR_COLOR && *(p_vmem - 2) == ' ')
                { //删除/t
                    int temp_limit = 4;
                    while (temp_limit > 0 && *(p_vmem - 2) == ' ' && *(p_vmem - 1) == ESC_CHAR_COLOR)
                    {
                        p_con->cursor--;
                        *(p_vmem - 2) = '\0';
    ...
    case '\t':
        ...
        {
            temp_loc = 8 - ((unsigned int)p_vmem) % 8; //对齐
            while (temp_loc != 0)
            {
                p_con->cursor++;
                *p_vmem = ' ';
